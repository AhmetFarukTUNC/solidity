...
describe('Lock Contract', function ()  {
    ...
    // Two contract deploys with before.
    ...
    // It is checked whether these two contracts are published or not

    it("Deploys the contracts",async function () {

         expect(token.address).to.not.be.undefined; // It checks if this address is defined.

         expect(lock.address).to.be.properAddress;// It checks if this address is defined.
    });
    ...
    describe('Contract Functions', function() { 
       ...
       it("user2 can not withdraw tokens",async function () { 
       ...
       it("user1 withdraw tokens",async function () { 
            ...
            expect(balances[3]-10).to.be.equal(ethToNum(await token.balanceOf(token.address)));
            expect(userLocks[0]).to.be.equal(ethToNum(await lock.lockers(user1.address)));
        });
       ...
       it("user1 can not withdraw more tokens",async function () { 
